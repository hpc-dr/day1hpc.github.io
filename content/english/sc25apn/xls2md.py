...
ICS_DIR = Path("ics")                      # output folder for .ics
LOCATION_STR = "AWS Booth #2207 in SC25 Expo"
TITLE_FMT = "{partner} Demo @ AWS Booth #2207 — {pod}"
DESCRIPTION_FMT = "Partner demo at {pod} during SC25 in St. Louis."
EMPTY_CELL_DISPLAY = ""                    # or "—"

# Accept month tokens like "No", "Nov", any case; be forgiving to "NO-17", "Nov 17", etc.
_MONTH_TOKENS = {"no", "nov"}

# Time row pattern:
#   "No-17 10.00-11.00"
#   "Nov 17 3pm-4pm"
#   "Nov-17 11:30 a.m.-12:15 p.m."
# Minutes optional; separator can be "-" or "–"; AM/PM optional.
TIME_PAT = re.compile(
    r"""
    ^\s*
...
def parse_slot(cell: str) -> tuple[datetime, datetime]:
    ...
    Parse strings like:
      ...
      - 'Nov-17 3pm-4pm'
      - 'Nov 17 11:30 a.m.-12:15 p.m.'
    Returns timezone-aware start/end datetimes.
    """
    ...
    def parse_time_token(token: str, ampm_raw: str | None) -> datetime:
        ...
        if ampm_raw:
            ampm = re.sub(r"[^aApPmM]", "", ampm_raw).lower()  # strip spaces/dots
    ...
    # If end <= start, assume it crosses midnight (unlikely, but safe)
    if end_dt <= start_dt:
...
def main():
    ...
    # Build ICS events by collapsing consecutive runs per pod+partner
    ICS_DIR.mkdir(parents=True, exist_ok=True)
    cell_link: dict[tuple[int, str], str] = {}  # (row_idx, pod) -> relative "ics/<file>.ics"
    ...
    for pod in pod_cols:
        ...
        def close_run():
            ...
            # Build ICS filename & relative link
            ...
            ics_url = f"{ICS_DIR.name}/{ics_filename}"
            ...
            # Assign link to all rows covered by this run
        ...
        for row_idx in idx_order:
            ...
            if partner_norm and current_partner_norm is None:
                ...
                if current_partner_norm is not None:
                    close_run()
                # Start new run if non-empty; otherwise reset
                if partner_norm:
                    current_partner_norm = partner_norm
                    current_partner_raw = partner_raw
                    run_start_idx = run_end_idx = row_idx
    ...
    for row_idx in idx_order:
        ...
        for pod in pod_cols:
            ...
            if link:
                md_row.append(f"[{safe_partner}]({link})")
            else:
                md_row.append(safe_partner)
        rows_md.append(md_row)

    md_table = format_markdown_table(headers, rows_md)

    # Output Markdown filename based on input path (in CWD)
